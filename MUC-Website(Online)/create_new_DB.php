...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $inflation_rate = trim($_POST['inflation_rate'] ?? '');

    if (!is_numeric($years) || !is_numeric($inflation_rate)) {
        $message = "Please enter valid numeric values.";
    } elseif ($years <= 0) {
        $message = "Please enter a positive number of years.";
    } else {
        $base_year = 2022;
        $query = "SELECT SUM(p.price * od.quantity) AS total_spent
                  FROM orders o
        ...
        $base_result = $stmt->fetch();
        $base_total = $base_result['total_spent'] ?? 0;

        if ($base_total == 0) {
            $message = "No expense data available for base year $base_year.";
        } else {
            // Convert inflation rate from percentage to decimal
            $rate = $inflation_rate / 100;
            // Calculate projection for next N years (from 2023 to 2022+N)
            for ($i = 1; $i <= $years; $i++) {
                $year = $base_year + $i;
                $projected = $base_total * pow(1 + $rate, $i);
                $projections[] = ['year' => $year, 'projected' => $projected];
            }
        }
    }
}
...
<head>
    <meta charset="UTF-8">
    <title>MUC Database - Create DB</title>
      ...
      <div class="account-info">
        ...
        <div class="app-content">
            <div class="app-content-header">
                <h1 class="app-content-headerText">Create/Delete Database</h1>
            ...
            <body>
                <div class="page-info">
                    <div class="page-title">
                        <h1>Create/Delete Database</h1>
                    </div>
                    
                    <form method="post" action="create_new_DB.php" class="page-form">
                        <h3>Create New Database:</h3>
                        <br>

                        <?php
                        if(isset($_POST['Add_DB_Name'])){
                             $sessid =  $_SESSION['UID'];;
                             //echo "1";
                            $credentials = explode(":", decrypt("keys/encKCred_$sessid.txt", "data/encDCred_$sessid.txt"));
                            $createQ = "CREATE DATABASE " . $_POST['Add_DB_Name'];
                            $totalDB = count(confirmDB("SHOW DATABASES;", $credentials[0], $credentials[1], $credentials[2]));

                            confirmDB($createQ, $credentials[0], $credentials[1], $credentials[2]);

                            if(count(confirmDB("SHOW DATABASES;", $credentials[0], $credentials[1], $credentials[2])) == $totalDB)
                            {
                                echo "<h2 style=\"color:red;\">There was an issue creating \"". $_POST['Add_DB_Name'] . "\"</h2>";

                            }
                            else {
                                echo "<h2 style=\"color:green;\"> Successfully Created \"" . $_POST['Add_DB_Name'] . "\"</h2>";
                            }

                        }
                        ?>
                        <label>New Database Name: </label><input type="text" name="Add_DB_Name" required placeholder="Enter DB name" > <br><br><br>



                        <input type='submit' value='Confirm New Database' >
                   ...
                   if(isset($_POST['removeTable'])){
                        $removeQ = "DROP DATABASE " . $_POST['removeTable'];

                        $totalDB = count(confirmDB("SHOW DATABASES;", $credentials[0], $credentials[1], $credentials[2]));
                        $dbnamee = $_POST['removeTable'];
                        confirmDB($removeQ, $credentials[0], $credentials[1], $credentials[2]);
                        ...
                        if(count(confirmDB("SHOW DATABASES;", $credentials[0], $credentials[1], $credentials[2])) == $totalDB)
                        {
                            ...
                            if ( $dbnamee == file_get_contents("ADB/ADB_$sessid.txt")){
          ...
          <div class="filter-menu" style="overflow: hidden; position: static; opacity: 1;" >

          <form method="post" action="create_new_DB.php" class="page-form" onsubmit="return WarnDelTable();">
              ...
              <h4>Select Database:</h4>
                        <select id="remtable" name="removeTable" required >
                            ...
                            $tbls = RequestDB("SHOW DATABASES;", $credentials[0], $credentials[1], $credentials[2]);
                            foreach ($tbls as $tbl) {
                            $tblName = implode(" ", $tbl) . "";
